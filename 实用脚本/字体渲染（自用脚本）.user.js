...
// @require         https://greasyfork.org/scripts/437214-frcolorpicker/code/frColorPicker.js?version=1011323
// @include         *
// @grant           GM_info
...
~(function (trustedTypesPolicy) {
  ...
  const timeStart = IS_OPEN_DEBUG ? console.time.bind(console) : () => {};
  const timeEnd = IS_OPEN_DEBUG ? console.timeEnd.bind(console) : () => {};
  ...
  const FONTLIST_IMG = defCon.decrypt("aHR0cHMlM0ElMkYlMkZ6My5heDF4LmNvbSUyRjIwMjElMkYwOSUyRjA0JTJGaDJLdWRLLmdpZg==");
  ...
  const getNavigator = {
    ...
    getBrowser: (brands, getBrand, info = "Other", version = "Unknown") => {
      if (getBrand) {
        brands.some(b => {
          switch (b.brand) {
            case "Microsoft Edge":
    ...
    getUA: async function (u = this.init()) {
      return this.uaData ? JSON.stringify(await u.getHighEntropyValues(["architecture", "bitness", "model", "platform", "platformVersion", "uaFullVersion"])) : u;
    },
  };
    ...
    try {
      if (_preview_) {
        ...
        qA("iframe").forEach(items => {
          try {
            items.setAttribute("sandbox", "allow-scripts allow-same-origin allow-top-navigation");
  ...
  const cache = {
    value: data => {
      const exp = 6048e5; // 7 days
      debug("\u27A4 cache expiration time: %s hrs", exp / 36e5);
      return {
        data: data,
    ...
    get: key => {
      ...
      if (!obj) {
        ...
        try {
          ...
          debug("\u27A4 cache expiration remaining: %s hrs", ((expiredTime - curTime) / 36e5).toFixed(3));
  ...
  const definePropertiesForZoom = function (t, d) {
    try {
      const setValue = (s, g, mod, z = 0) => {
        switch (mod) {
          case 0:
            if (s && s instanceof MouseEvent && s.target) {
              z = s.target.getBoundingClientRect()[g];
            }
            break;
          case 1:
            if (s && s instanceof MouseEvent && s.target.parentNode && s.target.parentNode !== document) {
              z = s.target.parentNode.getBoundingClientRect()[g];
            }
            break;
          case 2:
      ...
      Object.defineProperties(MouseEvent.prototype, {
        ...
        offsetX: {
          get: function () {
            return this.x / t - setValue(this, "left", 0);
          },
          configurable: false,
        },
        offsetY: {
          get: function () {
            return this.y / t - setValue(this, "top", 0);
          },
          configurable: false,
        },
        screenX: {
  ...
  !(async function FontRendering(loading) {
    ...
    if (curVersion !== defCon.curVersion && CUR_WINDOW_TOP) {
      addLoadEvents(() => {
        if (!isCloseTip) {
          hintUpdateInfo(`${HOST_URI}#update`, curVersion);
        }
    ...
    const defExS = ["workstation-xi"].sort();
    if (!exSite) {
      GMsetValue("_Exclude_site_", defCon.encrypt(JSON.stringify(defExS)));
      exSite = defExS;
    } else {
    ...
    let selection = "";
    if (stroke) {
      selection = `::selection{color:#ffffff!important;background:#0084ff!important}::-moz-selection{color:currentcolor!important;background:#1ebee34a!important}`;
    }
    let smoothing = "";
    ...
    const funcFontsize = t => {
      return `body{${getNavigator.core().Gecko ? `transform:scale(${t});transform-origin:left top 0px;width:${100 / t}%;height:${100 / t}%;` : `zoom:${t}!important;`}}`;
    };
    ...
    if (fontface_i) {
      fontfamily = `font-family:${CONST_VALUES.fontSelect};`;
      ...
      ? String(
          `<li id="${defCon.id.fontSize}">
            ...
            <div class="${defCon.class.range}" data-ticks-position="top"
              style="--min:.8;--max:1.5;--step:.001;--value:${CONST_VALUES.fontSize};--text-value:'${CONST_VALUES.fontSize.toFixed(3).toString()}'">
        ...
        <fieldset id="${defCon.id.field}" style="display:block">
          ...
          <ul class="${defCon.class.main}">
            ...
            <li id="${defCon.id.fontStroke}">
              ...
              <div class="${defCon.class.range}" data-ticks-position="top"
                style="--step:.001;--min:0;--max:1;--value:${CONST_VALUES.fontStroke};--text-value:'${CONST_VALUES.fontStroke.toFixed(3).toString()}'">
            ...
            <li id="${defCon.id.fontShadow}">
              ...
              <div class="${defCon.class.range}" data-ticks-position="top"
                style="--step:.01;--min:0;--max:8;--value:${CONST_VALUES.fontShadow};--text-value:'${CONST_VALUES.fontShadow.toFixed(2).toString()}'">
            ...
            <li id="${defCon.id.fontCSS}" style="min-width:254px">
              ...
              <textarea placeholder="请谨慎修改默认值，避免渲染失效。" id="${defCon.id.cssfun}" class="${defCon.class.readonly}"
                title="重要参数，默认只读，双击解锁。" readonly="readonly">${CONST_VALUES.fontCSS ? CONST_VALUES.fontCSS : INITIAL_VALUES.fontCSS}</textarea>
            </li>
            <li id="${defCon.id.fontEx}" style="min-width:254px">
              ...
              <textarea placeholder="请输入要排除渲染的HTML标签，形如: input, em, div[id='test']"
                id="${defCon.id.exclude}">${CONST_VALUES.fontEx ? CONST_VALUES.fontEx : ""}</textarea>
            </li>
    ...
    function correctBoldErrorByStroke(s) {
      return new Promise(resolve => {
        if (s && parseInt(getNavigator.chromiumVersion()) >= 96) {
          timeStart("\u27A4 [FIXSTROKE]");
          const ATT = "fr-fix-stroke";
          qA(
            `:not`.concat(
              `(`,
              `html,head,head *,base,meta,style,link,script,noscript,iframe,body,`,
              `img,form,br,hr,ul,canvas,applet,source,svg,clippath,rect,circle,path,`,
              `mask,filter,g,symbol,picture,embed,audio,video,track,figure,progress,[${ATT}],`,
              `fr-configure *,fr-dialogbox *,fr-colorpicker *,gb-notice *`,
              `)`
            )
          ).forEach(item => {
    ...
    try {
      ...
      new MutationObserver(mutations => {
        if (!((!CUR_WINDOW_TOP || qS(`.${defCon.class.rndClass}`)) && (defCon.siteIndex !== undefined || qS(`.${defCon.class.rndStyle}`)))) {
          debug(
            `\u27A4 %cMutationObserver: %c%s %c%s %c${getNavigator.core().Gecko ? `\n\u3000 \u27A6 ${CUR_HOST_NAME}` : ""}`,
            "color:teal",
            "color:olive",
            mutations[0].type,
            "color:red",
        ...
        defCon.siteIndex === undefined && deBounce(correctBoldErrorByStroke, 50, "fixstroke")(CONST_VALUES.fontStroke);
        ...
        let frDialog = new FrDialogBox({
          ...
          messageText: String(
            `<ul class="${defCon.class.main}" style="box-sizing:content-box;max-height:215px;overflow-x:hidden;margin:0;padding:5px 0">
              ...
              <li id="${defCon.id.fs}">
                <div class="${RANDOM_ID}_VIP" title="实验性功能\uff1a不推荐Firefox浏览器开启字体缩放\uff01">\u2462 字体缩放功能（默认\uff1a关闭）</div>
                <div style="margin:0;padding:0">
              ...
              <li id="${defCon.id.flc}">
                <div class="${RANDOM_ID}_VIP" title="安装新字体后，请先重启浏览器再重建缓存\uff01">\u2466 字体列表缓存（时效\uff1a一周）</div>
    ...
    if (CUR_WINDOW_TOP) {
      ...
      sleep(1e3, { useCachedSetTimeout: true })(2e3)
        ...
        .then((Font_Set, Feed_Back, Exclude_site, Parameter_Set) => {
          ...
          if (defCon.siteIndex === undefined) {
            ...
            Font_Set = GMregisterMenuCommand(`\ufff2\ud83c\udf13 字体渲染设置${isHotkey ? "(" + String.fromCharCode(IS_MACOS ? 81 : 80) + ")" : ""}`, () => {
            ...
            Parameter_Set = GMregisterMenuCommand(`\ufff7\ud83d\udc8e 高级核心功能设置${isHotkey ? "(" + String.fromCharCode(IS_MACOS ? 77 : 71) + ")" : ""}`, () => {
    ...
    if (isHotkey && CUR_WINDOW_TOP) {
      document.addEventListener("keydown", event => {
        ...
        if (e.keyCode === (IS_MACOS ? 81 : 80) && ekey) {
          e.preventDefault();
        ...
        if (e.keyCode === (IS_MACOS ? 77 : 71) && ekey) {
          e.preventDefault();
          if (Date.now() - defCon.clickTimer > 1e3) {
            defCon.clickTimer = Date.now();
            if (defCon.siteIndex === undefined) {
      ...
      try {
        if (isMutationObserver) {
          ...
          addLoadEvents(() => {
            ...
            const lastStyle = isLast || isPrevious;
            sleep(2e3)(lastStyle).then(r => {
              qS(`#${defCon.class.rndStyle}`) && document.head.lastChild.className !== defCon.class.rndStyle && insertStyle(r);
      ...
      try {
        if (CUR_WINDOW_TOP) {
          ...
          try {
            ...
            if (fontlist_Cache && Array.isArray(fontlist_Cache) && fontlist_Cache.length) {
          ...
          try {
            if (qS(`#${defCon.id.fontList} .${defCon.class.fontList}`)) {
              ...
              qS(`#${defCon.id.fonttooltip}`).addEventListener("dblclick", async (custom_Fontlist, save_Fontlist = DEFAULT_ARRAY, customFontlist = "") => {
                const cusFontList = await GMgetValue("_Custom_fontlist_");
                const cusFontCheck = cusFontList ? JSON.parse(defCon.decrypt(cusFontList)) : DEFAULT_ARRAY;
                if (cusFontCheck && Array.isArray(cusFontCheck) && cusFontCheck.length) {
                ...
                if (await frDialog.respond()) {
                  ...
                  } else if (fontListArray && Array.isArray(fontListArray) && fontListArray.length) {
          ...
          qS(`#${defCon.id.submit} .${defCon.class.submit}`).addEventListener("click", async function () {
            ...
            const prefzoom = isFontsize ? (/^[0-1](\.[0-9]{1,3})?$/.test(zoom.value) ? zoom.value : INITIAL_VALUES.fontSize) : 1;
            ...
            if (defCon.isPreview && this.getAttribute("v-Preview")) {
              ...
              try {
                ...
                domainValueIndex = updateDomainsIndex(domainValue);
                if (domainValueIndex !== undefined && qS(`#${RANDOM_ID}_c_w_d_`)) {
                ...
                defCon.fontlistchanged = fontselect.split(",")[0] !== CONST_VALUES.fontSelect.split(",")[0];
                if (await frDialog.respond()) {
                  saveData("_fonts_set_", {
            ...
            if (tfs && fs) {
              ...
              fs.addEventListener("change", () => {
                tfs.innerHTML = trustedTypesPolicy.createHTML(
                  `<em style="word-break:break-all;color:indigo;font-size:12px!important">${fs.files[0].name}</em><span style="color:crimson">\u0020\ud83d\udc49\u0020[重新选择]</span>`
                );
              });
            }
            if (await frDialog.respond()) {
      ...
      try {
        ...
        if (await frDialog.respond()) {
          ...
          let frDialog = new FrDialogBox({
            trueButtonText: "感谢使用",
            messageText: String(
              `<p style="color:darkgreen">网站个性化设置数据已保存\uff01${
    ...
    function reportErrorToAuthor(e, show = IS_OPEN_DEBUG, errors = "") {
      if (show) {
        closeConfigurePage(false);
        sleep(1e3).then(async () => {
          try {
            if (!document.querySelector("fr-dialogbox[error='true']")) {
              ...
              const copyText = qS(`#${RANDOM_ID}_copy_to_author`).textContent.replace(/\u3000/g, "\n");
    ...
    function addSingleQuoteToArray(arr, hasYH = false, returnStr = "") {
      if (arr && Array.isArray(arr) && arr.length) {
        ...
        if (hasYH || !checkFont.detect("Microsoft YaHei")) {
          returnStr += INITIAL_VALUES.fontSelect.replace(`'Microsoft YaHei',`, ``);
        } else {
          returnStr += INITIAL_VALUES.fontSelect;
        }
        checkFont = null;
      } else {
        returnStr = INITIAL_VALUES.fontSelect;
